...
assert (
    os.getenv("DISTDL_RELEASE_VERSION") != ""
), "DISTDL_RELEASE_VERSION should be either None or a valid string"
is_release = False
is_nightly = False
if os.getenv("DISTDL_RELEASE_VERSION"):
    release_version = os.getenv("DISTDL_RELEASE_VERSION")
    version = f"{release_version}"
    is_release = True
elif os.getenv("DISTDL_RELEASE_NIGHTLY"):
    today = date.today()
...
if not os.getenv("DISTDL_RELEASE_VERSION") and not os.getenv(
    "DISTDL_RELEASE_NIGHTLY"
):
