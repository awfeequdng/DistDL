...
parser.add_argument("--package_name", type=str, default="distdl")
...
def get_version():
    ...
    spec = importlib.util.spec_from_file_location(
        "version", os.path.join("distdl", "version.py")
    )
...
include_files = glob.glob("distdl/include/**/*", recursive=True)
include_files = [os.path.relpath(p, "distdl") for p in include_files]
assert len(include_files) > 0, os.path.abspath("distdl/include")


def get_distdl_internal_so_path():
    import imp

    fp, pathname, description = imp.find_module("_distdl_internal", ["distdl"])
    assert os.path.isfile(pathname)
    return os.path.relpath(pathname, "distdl")
...
setup(
    ...
    version=get_version(),
    url="https://www.distdl.org/",
    install_requires=REQUIRED_PACKAGES,
    packages=find_packages(),
    package_dir={"distdl": "distdl"},
