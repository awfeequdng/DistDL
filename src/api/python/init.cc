...
PYBIND11_MODULE(_distdl_internal, m) {
  using IntList = std::vector<int64_t>;
  using Int2IntListMap = std::unordered_map<int64_t, std::shared_ptr<IntList>>;

  py::module_ distdl_api_util = m.def_submodule("util");

  py::class_<IntList, std::shared_ptr<IntList>>(distdl_api_util, "IntList")
      .def(py::init<>())
  ...
  py::class_<Int2IntListMap, std::shared_ptr<Int2IntListMap>>(distdl_api_util, "Int2IntListMap")
      .def(py::init<>())
  ...
  ::distdl::DistDLModuleRegistry::ImportAll(m);
}
